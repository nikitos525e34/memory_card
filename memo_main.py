...
######################################              Константи:              #############################################
main_width, main_height = 1100, 950  # початкові розміри головного вікна
card_width, card_height = 1000, 700  # початкові розміри вікна "картка"
...
def testlist():
    ...
    frm = Form('Яка з цих моделей є електричним автомобілем BMW?', 'BMW i3',
               'BMW X5', 'BMW 3 series', 'BMW Z4')
    questions_listmodel.form_list.append(frm)

    frm = Form('Що означає бревіатура BMW?', 'Баварські моторні роботи',
               'Британский моторний трнспорт', 'Найкращі моторні колеса', 'Берлінський моторний світ')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка з цих моделей BMW є спортивним автомобілем?', 'BMW M3',
               'BMW 5 series', 'BMW X3', 'BMW 7 series')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка система керуваня автомобілем вперше була впроваджена у BMW?', 'ABS',
               'XDrive', 'DSC', 'ESC')
    questions_listmodel.form_list.append(frm)

    frm = Form('Який двигун використовує BMW i8?', 'Гібрідний',
               'Чотирициліндровий', 'Двотурбінний V8 ', 'Дизельний')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка з моделей BMW вважається позашляховиком?', 'BMW X7',
               'BMW 1 series', 'BMW M5', 'BMW Z3')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка з цих технологій не є характерною для BMW?', 'Автопілот',
               'Адаптивний круїз контроль', 'Динамічна стабільність', 'Проектор на лобове скло')
    questions_listmodel.form_list.append(frm)

    frm = Form('Яка технологія дозволяє BMW оптимізувати витрати пального?', 'Технологія старт-стоп',
               'Гібридний привід', 'Еко-режим', 'Турбонаддув')
    questions_listmodel.form_list.append(frm)
